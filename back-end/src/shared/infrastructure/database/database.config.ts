...
const options: DataSourceOptions & SeederOptions = {
  ...
  port: process.env.MYSQL_TCP_PORT ? parseInt(process.env.MYSQL_TCP_PORT) : 3306,
  username: `${process.env.MYSQL_USER}`,
  password: `${process.env.MYSQL_PASSWORD}`,
  database: `${process.env.MYSQL_DATABASE}`,
  entities: [join(__dirname, '../../../modules/**/*.entity.orm.{ts,js}')],
  logging: process.env.NODE_ENV === 'local' ? String(process.env.TYPEORM_LOGGING).toLowerCase() === 'true' : false,
  synchronize:
